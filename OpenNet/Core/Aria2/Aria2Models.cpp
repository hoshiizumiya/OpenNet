...
OpenNet::Core::Aria2::PieceMapInformation OpenNet::Core::Aria2::BuildPieceMapInformation(DownloadInformation const& value)
{
	PieceMapInformation result;
	result.PieceLength = value.PieceLength > 0 ? value.PieceLength : std::size_t{ 1024 * 1024 };
	auto const pieceCount = value.NumPieces > 0
		? value.NumPieces
		: value.TotalLength > 0
		? (value.TotalLength + result.PieceLength - 1) / result.PieceLength
		: std::size_t{};
	result.States.assign(pieceCount, 0);

	if (value.Bitfield.empty())
	{
		auto const completedPieces = (std::min)(pieceCount, value.CompletedLength / result.PieceLength);
		std::fill_n(result.States.begin(), completedPieces, 2);
		return result;
	}

	auto const hexValue = [](char const digit)
	{
	...
	for (std::size_t index = 0; index < pieceCount; ++index)
	{
		...
		if (nibble < value.Bitfield.size()
			&& (hexValue(value.Bitfield[nibble]) & (1 << (3 - static_cast<int>(index % 4)))) != 0)
		{
			result.States[index] = 2;
		}
	}
	return result;
}
