...
export namespace Core::Utils::Misc
{
	...
	winrt::hstring friendlyUnit(std::int64_t bytes, bool isSpeed = false, int precision = -1);
	winrt::hstring friendlyUnitCompact(std::int64_t bytes);
	int friendlyUnitPrecision(SizeUnit unit);
	std::int64_t sizeInBytes(double size, SizeUnit unit);
	...
	winrt::hstring userFriendlyDuration(std::int64_t seconds, std::int64_t maxCap = -1, TimeResolution resolution = TimeResolution::Minutes);
	winrt::hstring FormatTimestamp(int64_t timestamp);
