...
namespace OpenNet::Core::RSS
{
    ...
    void RSSDatabase::Open()
    {
        try
        {
            ...
            auto dbPath = std::wstring(localFolder.Path().c_str()) + L"\\rss_data.db";
            auto dbPathUtf8 = WideToUtf8(dbPath);

            int rc = sqlite3_open(dbPathUtf8.c_str(), &m_db);
            if (rc != SQLITE_OK)
            {
                OutputDebugStringA(("RSSDatabase: failed to open DB – " + std::string(sqlite3_errmsg(m_db)) + "\n").c_str());
                m_db = nullptr;
                return;
            }
        ...
        {
            OutputDebugStringA(("RSSDatabase::Open exception: " + std::string(ex.what()) + "\n").c_str());
        }
    }
