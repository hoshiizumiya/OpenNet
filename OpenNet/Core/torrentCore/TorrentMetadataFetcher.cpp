...
#define m_errorMessage m_impl->errorMessage
	...
	std::optional<TorrentMetadataInfo> TorrentMetadataFetcher::ParseTorrentFile(std::string const& filePath)
	{
		try
		{
			auto const settings = ::OpenNet::Core::TorrentSettingsManager::Instance().Get();
			lt::load_torrent_limits limits;
			limits.max_directory_depth = std::clamp(settings.maxTorrentDirectoryDepth, 1, 1000);
			auto params = lt::load_torrent_file(filePath, limits);
