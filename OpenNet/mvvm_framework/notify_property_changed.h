...
#pragma region NotifyPropertyChanged Common Functions
		...
		winrt::event_token ValidationRequested(auto const& handler)
		{
			return m_eventValidationRequested.add(handler);
		}
		void ValidationRequested(winrt::event_token const& token)
		{
			m_eventValidationRequested.remove(token);
		}

		winrt::event_token ValidationCompleted(auto const& handler)
		{
			return m_eventValidationCompleted.add(handler);
		}
		void ValidationCompleted(winrt::event_token const& token)
		{
			m_eventValidationCompleted.remove(token);
		}

		winrt::event_token ErrorsChanged(auto const& handler)
		{
			return m_eventErrorsChanged.add(handler);
		}
		void ErrorsChanged(winrt::event_token const& token)
		{
			m_eventErrorsChanged.remove(token);
		}
	private:
