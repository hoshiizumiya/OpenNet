...
namespace winrt::OpenNet::Pages::implementation
{
	...
	winrt::Windows::Foundation::IAsyncAction TasksPage::ShowTaskPropertiesAsync()
	{
		try
		{
			...
			// TODO: 创建并显示任务属性对话框
			// auto propertiesDialog = make<TaskPropertiesDialog>(m_currentSubscribedTask);
			// auto result = co_await propertiesDialog.ShowAsync();

			// 临时实现：显示调试输出
			auto taskName = m_currentSubscribedTask.Name();
			auto size = m_currentSubscribedTask.Size();
			auto progress = m_currentSubscribedTask.Progress();
