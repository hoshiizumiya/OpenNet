...
namespace winrt::OpenNet::UI::Xaml::Media::implementation
{
		...
		DependencyPropertyChangedEventArgs const& args)
	{
		...
		{
			auto const self = get_self<AcrylicBrush>(brush);

			// Noise is already part of the effect graph. Match MUX and only
			// change its opacity instead of rebuilding the entire acrylic brush.
			if (args.Property() == s_isNoiseEnabledProperty)
			{
				if (self->m_brush)
				{
					if (auto effectBrush = self->m_brush.try_as<CompositionEffectBrush>())
					{
						effectBrush.Properties().InsertScalar(
							NoiseOpacityPropertyName,
							self->IsNoiseEnabled() ? NoiseOpacity : 0.0f);
					}
				}
				return;
			}
	...
	Windows::UI::Color AcrylicBrush::TintColor() const
	{
		return unbox_value<Windows::UI::Color>(GetValue(s_tintColorProperty));
	}
	void AcrylicBrush::TintColor(Windows::UI::Color const& value)
	{
		SetValue(s_tintColorProperty, box_value(value));
	}
	DependencyProperty AcrylicBrush::TintColorProperty()
	{
	...
	{
		SetValue(s_tintOpacityProperty, box_value(std::clamp(value, 0.0, 1.0)));
	}
	DependencyProperty AcrylicBrush::TintOpacityProperty()
	{
		return s_tintOpacityProperty;
	}

	IReference<double> AcrylicBrush::TintLuminosityOpacity() const
	{
		return GetValue(s_tintLuminosityOpacityProperty).try_as<IReference<double>>();
	}
	void AcrylicBrush::TintLuminosityOpacity(IReference<double> const& value)
	{
		SetValue(s_tintLuminosityOpacityProperty, value);
	}
	DependencyProperty AcrylicBrush::TintLuminosityOpacityProperty()
	{
		return s_tintLuminosityOpacityProperty;
	}

	TimeSpan AcrylicBrush::TintTransitionDuration() const
	{
		return unbox_value<TimeSpan>(GetValue(s_tintTransitionDurationProperty));
	}
	void AcrylicBrush::TintTransitionDuration(TimeSpan const& value)
	{
		SetValue(s_tintTransitionDurationProperty, box_value(value));
	}
	DependencyProperty AcrylicBrush::TintTransitionDurationProperty()
	{
		return s_tintTransitionDurationProperty;
	}

	bool AcrylicBrush::AlwaysUseFallback() const
	{
		return unbox_value<bool>(GetValue(s_alwaysUseFallbackProperty));
	}
	void AcrylicBrush::AlwaysUseFallback(bool value)
	{
		SetValue(s_alwaysUseFallbackProperty, box_value(value));
	}
	DependencyProperty AcrylicBrush::AlwaysUseFallbackProperty()
	{
		return s_alwaysUseFallbackProperty;
	}

	bool AcrylicBrush::IsNoiseEnabled() const
	{
		return unbox_value<bool>(GetValue(s_isNoiseEnabledProperty));
	}
	void AcrylicBrush::IsNoiseEnabled(bool value)
	{
		SetValue(s_isNoiseEnabledProperty, box_value(value));
	}
	DependencyProperty AcrylicBrush::IsNoiseEnabledProperty()
	{
		return s_isNoiseEnabledProperty;
	}
	...
	void AcrylicBrush::RebuildBrush()
	{
		...
		try
		{
			...
			if (tintColor.A != 255)
			{
				...
				// Win2D maps Background to D2D input 0 (source) and Foreground
				// to input 1 (destination). Color and Luminosity therefore have
				// opposite observable behavior here; keep the MUX workaround.
				luminosityBlend.Mode(BlendEffectMode::Color);
				...
				colorBlend.Mode(BlendEffectMode::Luminosity);
			...
			noiseOpacity.Opacity(NoiseOpacity);
