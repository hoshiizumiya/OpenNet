...
namespace winrt::OpenNet::Models::implementation
{
	...
	{
		std::uint16_t listenPort{ 6881 };       // 监听端口 / Listen Port
		std::uint16_t minPort{ 49152 };         // 最小端口范围 / Min Port Range
		std::uint16_t maxPort{ 65535 };         // 最大端口范围 / Max Port Range
		bool randomPort{ true };           // 随机端口 / Random Port
		bool useUPnP{ true };              // 使用UPnP / Use UPnP
		std::vector<std::uint16_t> customPorts{}; // 自定义端口
		std::vector<std::uint16_t> blockedPorts{}; // 阻塞端口
	};
	...
	struct FirewallStatus
	{
		...
		std::vector<std::uint16_t> allowedPorts{};     // 允许的端口
		std::vector<std::uint16_t> deniedPorts{};      // 拒绝的端口
