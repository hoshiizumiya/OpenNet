...
namespace winrt::OpenNet::Models
{
	...
	struct PeerInfo
	{
		...
		std::uint16_t localPort;                // 本地端口 / Local Port
		std::uint16_t publicPort;               // 公网端口 / Public Port
		...
		std::int32_t signalStrength;                // 信号强度(0-100) / Signal Strength (0-100)
		...
		std::uint64_t maxFileSize;            // 最大文件大小 / Max File Size
		std::uint32_t maxConnections;         // 最大连接数 / Max Connections

		// 统计信息 / Statistics
		std::uint64_t bytesReceived;        // 接收字节数 / Bytes Received
		std::uint64_t bytesSent;            // 发送字节数 / Bytes Sent
		std::uint32_t filesReceived;        // 接收文件数 / Files Received
		std::uint32_t filesSent;            // 发送文件数 / Files Sent
		std::uint32_t connectAttempts;      // 连接尝试次数 / Connection Attempts
		std::uint32_t successfulConnects;   // 成功连接次数 / Successful Connections
		...
		PeerInfo()
			...
			, supportsResume(true)
			, maxFileSize(std::numeric_limits<std::uint64_t>::max())
			, maxConnections(10)
			, bytesReceived(0)
			, bytesSent(0)
			, filesReceived(0)
